...
- [x] Part Window 배치 버튼 접근성 라벨과 키보드 너비 조절
- [ ] 전체 화면 키보드 탐색
- [ ] 고해상도/DPI
- [ ] 다크·라이트 테마
- [ ] 대형 문서 virtualize
...
- [x] Codex App Server thread에 프로젝트 한정 `search_project_chunks`와 `read_project_chunks` 동적 도구를 제공한다.
- [x] 페어 작가가 현재 출처만으로 부족할 때 검색 후보를 확인하고 필요한 청크 본문만 읽을 수 있다.
- [x] 검색·읽기 횟수, 자동 청크 수, 본문 byte와 남은 작업 프로필 토큰 예산을 Core가 제한한다.
- [x] 명시 선택 출처를 우선하고 자동 참조 출처·revision·UTF-8 byte·line·당시 원문을 사용자 메시지 snapshot에 확정한다.
- [x] 도구 결과의 프로젝트 본문을 참고 자료로만 취급하고 그 안의 문장을 시스템 지시로 따르지 않도록 지시한다.
- [x] 동적 도구 미지원 Codex에서는 기존 명시 선택 컨텍스트로 후퇴한다.

TARGET:

- [ ] 원본 문서와 분리된 장기 기억 저장소를 정의한다.
- [ ] 기억의 근거·종류·생성 시점·유효 상태를 저장하고 사용자가 수정·비활성화·삭제할 수 있게 한다.
- [ ] 문서 청크 도구와 같은 프로젝트 경계로 장기 기억 검색·읽기 도구를 추가한다.
- [ ] 키워드 검색을 보완하는 의미 검색·벡터 색인은 별도 검증 뒤 선택한다.
- [ ] 저비용 경량 모델을 앞단 검색 라우터로 둘지는 Codex 단독 흐름의 품질·지연·사용량을 측정한 뒤 결정한다.

- [x] AI가 원본 파일을 즉시 수정하지 않는다.
- [x] AI 요청에 포함된 출처를 보존한다.
- [x] 응답과 변경 제안을 연결한다.
