...
상태: `CURRENT` (프로젝트·문서 저장 + AI 공동 집필 제안 + Instance 레이아웃 저장)
...
- 문서·자산·채팅·페르소나 저장
- Instance Part Window 순서·최소화·너비와 공동 집필 Part Section 순서·접힘·높이의 v2 설정 저장, 기존 v1 설정 마이그레이션
- 안정적인 ID와 revision
...
`src/instance-layout-store.ts`는 프로젝트·문서·편집기·공동 집필의 순서, 접힘 상태와 CSS px 너비를 `dataRoot/settings/instance-layout.json`에 검증·원자 저장한다. 동시 저장은 호출 순서로 직렬화하며 손상된 파일은 조용히 초기화하지 않는다.
...
현재 명령은 health/readiness, Browser session, `instance.layout.read/update`, 프로젝트·문서 CRUD, 채팅 읽기·전송, `chat.history.list`, `chat.context.preview`, `chat.message.classification.update`, proposal 적용·거절과 `instance.shutdown.request`다. 이전 Browser 호환을 위해 `system.shutdown.request`도 같은 Instance Runtime 종료 별칭으로 유지한다. 읽기 전용 잠금, 기존 폴더 연결, rename/move/delete, 부분 diff 승인과 원복은 후속 목표다.
