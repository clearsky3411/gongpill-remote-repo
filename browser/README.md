...
현재 프로젝트·문서·편집기·공동 집필 영역은 접기·펼치기, 좌우 순서 이동, 포인터·키보드 너비 조절과 데이터 루트 단위 복원을 지원한다. 자유 부동 창과 임의 재도킹, 프로젝트별 레이아웃은 후속 `TARGET`이다. Package 영역은 실제 Package UI가 생길 때 같은 계약에 등록하며 현재 화면에 가짜 영역을 만들지 않는다.
