...
Browser 테스트는 Instance Part Window·공동 집필 Part Section v2 레이아웃의 기본값, 입력 경계, v1 마이그레이션, 원자 저장, 동시 저장 직렬화와 손상 파일 처리에 더해 Browser/Core 기본값 일치, 최소화·접기·순서 이동·인접 크기 조절 순수 함수와 작은 제목 바·드래그·스냅·가로/세로 손잡이 DOM 연결을 검증한다.
...
- `tests/browser/instance-panel-layout.test.ts`
