...
- [x] 청크 파싱, 증분 색인, 명시 선택과 Codex 페어 작가 자동 검색·읽기 컨텍스트 조립
- [ ] 변경 제안, diff, 승인, 적용, 감사와 원복
- [ ] 실행 Flow/Scope/Trace, 진행률, 취소와 오류 추적
- [ ] 플러그인 계약·SDK·권한·설치 관리자
- [ ] 플러그인별 backend 프로세스와 sandboxed UI 격리
- [x] 작은 창 형태의 Part Window 제목 바, 화면 내부 스냅 재배치와 공동 집필 Part Section 조작 UI
- [ ] 프로젝트별 레이아웃 저장
- [ ] 지도 뷰어 검증 플러그인
- [x] 채팅·페르소나·명시 선택·페어 작가 자동 청크 출처 추적
- [ ] 브랜치와 원본에서 분리된 장기 기억 저장·검색·수정·삭제
...
- [x] 같은 Codex 턴에 프로젝트 청크 검색·읽기 동적 도구를 제공하고 도구 호출 횟수·청크 수·토큰·크기를 Core에서 제한한다
- [x] 동적 도구 미지원 Codex 버전에서는 기존 명시 선택 컨텍스트로 안전하게 후퇴한다
...
- [x] Codex 페어 작가가 현재 출처로 부족한 프로젝트 사실을 키워드 검색하고 후보 중 필요한 청크만 읽는다
- [x] 자동 읽기는 프로젝트 경계를 벗어나지 않고 요청당 검색 3회·읽기 3회·청크 6개·본문 128KB·남은 프로필 토큰 예산으로 제한한다
- [x] 명시 선택 청크를 자동 참조보다 우선하고 중복·예산 제외 이유를 snapshot에 남긴다
- [x] 사용자 메시지에 검색 횟수와 실제 자동 참조 청크의 revision·줄·byte·당시 원문을 저장해 다시 확인한다
- [x] 2026-07-30 `validate:release`의 73개 테스트와 portable·installer·architecture 전체 게이트가 통과한다
...
- [ ] 청크를 직접 선택하지 않고 다른 문서의 사실을 질문해 `페어 작가 자동 참조`가 표시되는지 확인한다
- [ ] 일반 대화에서는 불필요한 자동 검색이 실행되지 않는지 확인한다
